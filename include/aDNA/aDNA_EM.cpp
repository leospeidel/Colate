...
void get_ABC(int num_epochs, std::vector<double>& coal_rates, std::vector<double>& t_int, std::vector<int>& ep_index, std::vector<double>& A, std::vector<double>& B, std::vector<double>& C){
...
EM_shared(std::vector<double>& coal_rates, float age_begin, float age_end, float tmrca, std::vector<float>& epochs, std::vector<float>& num, std::vector<float>& denom){
  ...
  for(; e < ep_index[i_end]+1; e++){
    ...
    if(normalising_constant == 0.0){
      ...
      if(normalising_constant < f[e]){
        normalising_constant = f[e] + log(1.0 + exp(normalising_constant-f[e]));
      }else{
        normalising_constant = normalising_constant + log(1.0 + exp(f[e]-normalising_constant));
      }
    }
