...
mut_tree_fast(cxxopts::Options& options){
  ...
  *it_age_bin = 0;
  ...
  for(int chr = 1; chr <= 10; chr++){
    ...
    MarginalTree mtr; //stores marginal trees. mtr.pos is SNP position at which tree starts, mtr.tree stores the tree
    Muts::iterator it_mut; //iterator for mut file
    AncMutIterators ancmut("chr_" + std::to_string(chr) + "/" + options["anc"].as<std::string>(), 
        "chr_" + std::to_string(chr) + "/" + options["mut"].as<std::string>());
    haps input(("chr_" + std::to_string(chr) + "/" + options["input"].as<std::string>() + ".haps.gz").c_str(), 
        ("chr_" + std::to_string(chr) + "/" + options["input"].as<std::string>() + ".sample.gz").c_str());
