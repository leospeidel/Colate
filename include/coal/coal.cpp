...
mut(cxxopts::Options& options){
	...
	if(!options.count("mut") || !options.count("target_vcf") || !options.count("reference_vcf") || !options.count("output")){
		std::cout << "Not enough arguments supplied." << std::endl;
		std::cout << "Needed: mut, haps, sample, input, output. Optional: num_bins, coal" << std::endl;
		help = true;
	}
	if(options.count("help") || help){
		std::cout << options.help({""}) << std::endl;
  ...
  double log_10 = std::log(10);
	...
	*it_age_bin = 0.0;
	...
	std::vector<int> age_shared_count(num_age_bins*num_age_bins, 0), age_notshared_count(num_age_bins*num_age_bins, 0);
...
mut_fast_simplified_all(cxxopts::Options& options){
	...
	int num_used_snps = 0, num_used_snps2 = 0, num_used_snps3 = 0;
	std::vector<std::string> chromosomes;
	if(options.count("chr") > 0){
