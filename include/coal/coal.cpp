...
mut(cxxopts::Options& options){
  ...
  bool help = false;
  if(!options.count("mut") || !options.count("target_vcf") || !options.count("reference_vcf") || !options.count("output")){
    std::cout << "Not enough arguments supplied." << std::endl;
    std::cout << "Needed: mut, target_vcf, reference_vcf, output. Optional: num_bins, coal" << std::endl;
    help = true;
  }
  if(options.count("help") || help){
    std::cout << options.help({""}) << std::endl;
  ...
  if(options.count("coal") > 0){
    ...
    std::cerr << (log_age == 10000/28.0) << std::endl;
  ...
  *it_age_bin = 0.0;
  ...
  ///////////
  std::vector<std::string> filename_mut, filename_target, filename_ref;
  if(options.count("chr") > 0){
        ...
        for(int bin2 = bin1; bin2 < num_age_bins; bin2++){
          ...
          if(age_shared_count[bin] > 0){ 
            ...
            for(int e = 0; e < num_epochs; e++){
              ...
              assert(num[e] >= 0.0);
              assert(denom[e] >= 0.0);
              //if(is_ancient && e == 0) std::cerr << bin1 << " " << bin2  << " " << age_shared_count[bin] << " "  << num[e] << " " << num[e+1] << " " << num[e+2] << " " << denom[e] << " " << denom[e+1] << " " << denom[e+2] << " | " << num[e]/denom[e] << " " << num[e+1]/denom[e+1] << " " << num[e+2]/denom[e+2]  << std::endl;
              //if(is_ancient && e == 0) std::cerr << bin1 << " " << bin2 << " "  << num[e] << " " << num[e+1] << " " << age_shared_count[bin] << " " << denom[e] << std::endl;
          ...
          if(age_notshared_count[bin] > 0){ 
            ...
            for(int e = 0; e < num_epochs; e++){
              ...
              assert(num[e] >= 0.0);
              assert(denom[e] >= 0.0);
              //if(is_ancient && e == 0) std::cerr << bin1 << " " << bin2  << " " << age_notshared_count[bin] << " "  << num[e] << " " << num[e+1] << " " << num[e+2] << " " << denom[e] << " " << denom[e+1] << " " << denom[e+2] << " | " << num[e]/denom[e] << " " << num[e+1]/denom[e+1] << " " << num[e+2]/denom[e+2]  << std::endl;
              coal_rates_num[e]   += count * num[e];
              coal_rates_denom[e] += count * denom[e];
            }
          }
