              ...
              if(bp_target < bp_mut){
                while(target.read_snp() == 0){
                  ...
                  for(int i = 0; i < target.n*target.ploidy; i++){
                    DAF_check += (bcf_gt_allele(target.gt[i]) > 0);
                  }
                ...
                if(ancestral_target == derived && derived_target == ""){
                  ...
                  for(int i = 0; i < target.n; i++){
                    for(int j = 0; j < target.ploidy; j++){
                      if(k % 2 == choose){
                        DAF_target += (bcf_gt_allele(target.gt[target.ploidy*i+j]) > 0);
                        N_target += (bcf_gt_allele(target.gt[target.ploidy*i+j]) >= 0);
                      }else{
                        DAF_ref += (bcf_gt_allele(target.gt[target.ploidy*i+j]) > 0);
                        N_ref += (bcf_gt_allele(target.gt[target.ploidy*i+j]) >= 0);
                      }
                      k++;
                ...
                if( (ancestral_ref == ancestral && derived_ref == derived) || (ancestral_ref == derived && derived_ref == ancestral) ){
                  ...
                  for(int i = 0; i < ref.n; i++){
                    for(int j = 0; j < ref.ploidy; j++){
                      DAF_ref += (bcf_gt_allele(ref.gt[ref.ploidy*i+j]) > 0);
                      N_ref += (bcf_gt_allele(ref.gt[ref.ploidy*i+j]) >= 0);
...
calc_depth(cxxopts::Options& options){
  ...
  for(int chr = 0; chr < name_chr.size(); chr++){
    ...
    int bp_target = 3574476 - 1;
    target.read_to_pos(bp_target+1);

    std::cerr << target.count_alleles[bp_target % target.num_entries][0] << std::endl;
    std::cerr << target.count_alleles[bp_target % target.num_entries][1] << std::endl;
    std::cerr << target.count_alleles[bp_target % target.num_entries][2] << std::endl;
    std::cerr << target.count_alleles[bp_target % target.num_entries][3] << std::endl;
  ...
  while(vcf.read_snp() == 0){
    ...
    for(int i = 0; i < vcf.n; i++){
      for(int j = 0; j < vcf.ploidy; j++){
        DAF += (bcf_gt_allele(vcf.gt[vcf.ploidy*i+j]) > 0);
        N += (bcf_gt_allele(vcf.gt[vcf.ploidy*i+j]) >= 0);
...
count_topo(cxxopts::Options& options){
  ...
  std::cerr << "---------------------------------------------------------" << std::endl;
  std::cerr << "Counting topo for " << options["target_tmp"].as<std::string>() << " " << options["reference_tmp"].as<std::string>() << ".." << std::endl;
...
void
GetConditionalCoalescentRate(Tree& tree, int& BP, Sample& sample, std::vector<Leaves>& leaves, double factor, std::vector<int>& focal_haps, std::vector<int>& conditional_haps, std::vector<float>& epoch, std::vector<float>& epoch_focal, std::vector<CollapsedMatrix<double>>& coalescence_rate_num, std::vector<CollapsedMatrix<double>>& coalescence_rate_denom){
...
void
GetConditionalCoalescentRate(Tree& tree, Sample& sample, std::vector<Leaves>& leaves, double factor, std::vector<int>& focal_haps, std::vector<int>& conditional_haps, std::vector<float>& epoch, std::vector<float>& epoch_focal, std::vector<double>& sample_ages, std::vector<CollapsedMatrix<double>>& coalescence_rate_num, std::vector<CollapsedMatrix<double>>& coalescence_rate_denom){
  ...
  //std::cerr << coalescence_rate_num[0][0][0] << " " << coalescence_rate_denom[0][0][0] << " " << coalescence_rate_num[0][0][0]/coalescence_rate_denom[0][0][0] << std::endl;
}
...
CondCoalRates(cxxopts::Options& options){
  ...
  if(!options.count("input") || !options.count("output")){
    std::cout << "Not enough arguments supplied." << std::endl;
    std::cout << "Needed: input, output, groups, poplabels, chr. Optional: years_per_gen, dist, bins." << std::endl;
    help = true;
  }
  if(options.count("help") || help){
    std::cout << options.help({""}) << std::endl;
  ...
  std::vector<std::vector<CollapsedMatrix<double>>> coalescence_rate_num(num_blocks), coalescence_rate_denom(num_blocks);
    ...
    if(ancmut.sample_ages.size() > 0){
      ...
      while(num_bases_tree_persists >= 0.0){
        ...
        if(num_passing >= cutoff){
          ...
          factor = num_bases_tree_persists/1e6;
  ...
  for(int iter = 0; iter < num_bootstrap; iter++){
    ...
    std::vector<CollapsedMatrix<double>> res_num(num_epochs), res_denom(num_epochs);
    for(int e = 0; e < epochs_focal.size(); e++){
    ...
    for(int block = 0; block < num_blocks; block++){
      ...
      for(int e = 0; e < epochs_focal.size(); e++){
        std::vector<double>::iterator it_coal_num = coalescence_rate_num[block][e].vbegin();
        for(std::vector<double>::iterator it_num = res_num[e].vbegin(); it_num != res_num[e].vend(); it_num++){
          (*it_num) += blocks[block] * (*it_coal_num);
          it_coal_num++;
        }
        std::vector<double>::iterator it_coal_denom = coalescence_rate_denom[block][e].vbegin();
        for(std::vector<double>::iterator it_denom = res_denom[e].vbegin(); it_denom != res_denom[e].vend(); it_denom++){
