...
void
coal_LA::init_bootstrap(int seed){

  rng.seed(1);
  std::uniform_int_distribution<int> d(0, num_blocks-1);
  std::vector<int> tmp(num_blocks);
  blocks.resize(num_bootstrap);
  for(it1_blocks = blocks.begin(); it1_blocks != blocks.end(); it1_blocks++){
...
void 
coal_LA::Dump(std::string filename, std::vector<std::string>& unique_groups, int seed){
  ...
  for(it1_blocks = blocks.begin(); it1_blocks != blocks.end(); it1_blocks++){
    ...
    (*it1_num_boot).resize(num_groups);
    for(int i = 0; i < num_groups; i++){
      (*it1_num_boot)[i].resize(num_groups);
      for(int j = 0; j < num_groups; j++){
        (*it1_num_boot)[i][j].resize(num_epochs);
    ...
    for(int i = 0; i < num_groups; i++){
      ...
      for(int j = 0; j < num_groups; j++){
        (*it1_denom_boot)[i][j].resize(num_epochs);
